...
def main(source_files, extent_dest=None, EPSG_dest=None, start = '20150101', end = datetime.datetime.today().strftime('%Y%m%d'), resolution = 0, correct_mask = True, cloud_buffer = 0., colour_balance = False, processes = 1, output_dir = os.getcwd(), output_name = 'mosaic', masked_vals = 'auto', temp_dir = '/tmp', verbose = False):
    ...
    srefs = []
    srefs_wkt = []
    # The most popular projection
    for file in source_files:
        b04_10m = glob.glob(f"{file}\\IMG_DATA\\R10m\\*B04_10m.jp2")[0]

        raster = gdal.Open(b04_10m)
        sref = osr.SpatialReference()
        wkt = raster.GetProjection()
        sref.ImportFromWkt(wkt)

        srefs.append(sref.GetAttrValue("PROJCS|AUTHORITY", 1))
        srefs_wkt.append(wkt)
    
    epsg_votes = list(Counter(srefs).values())
    epsg_votes_max = epsg_votes.index(max(epsg_votes))
    epsg = srefs[epsg_votes_max]
    epsg_wkt = srefs_wkt[epsg_votes_max]


    # Initial bounding box (set to global scale in meters)
    bounds = [40075000, 1383200, 0, 0]
    ...
    for file in source_files:
        ...
        raster = gdal.Open(b04_10m)
        sref = osr.SpatialReference()
        sref.ImportFromWkt(raster.GetProjection())
        dest_sref = osr.SpatialReference()
        dest_sref.ImportFromWkt(epsg_wkt)
        ...
        # Transform to most popular epsg
        transform = osr.CoordinateTransformation(sref, dest_sref)
        tl, tt, garbage = transform.TransformPoint(ulx, uly)
        tr, tb, garbage = transform.TransformPoint(lrx, lry)

        if tl < bounds[0]: bounds[0] = tl
        if tb < bounds[1]: bounds[1] = tb
        if tr > bounds[2]: bounds[2] = tr
        if tt > bounds[3]: bounds[3] = tt

    if extent_dest is None:
        extent_dest = bounds
    
    if EPSG_dest is None:
        EPSG_dest = int(epsg)
...
if __name__ == "__main__":
    ...
    # Optional arguments
    optional.add_argument('-te', '--target_extent', nargs = 4, metavar = ('XMIN', 'YMIN', 'XMAX', 'YMAX'), type = float, help = "Extent of output image tile, in format <xmin, ymin, xmax, ymax>.")
    optional.add_argument('-e', '--epsg', metavar = 'EPSG', type=int, help="EPSG code for output image tile CRS. This must be UTM. Find the EPSG code of your output CRS as https://www.epsg-registry.org/.")
