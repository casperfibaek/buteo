...
def _get_stats(block):
    ...
    for fid in range(start, stop):
        ...
        temp_vector_layer = temp_vector_dataSource.CreateLayer('poly', vector_projection, ogr.wkbPolygon)
        temp_feature = vector_layer.GetFeature(fid)
        temp_vector_layer.CreateFeature(temp_feature.Clone())
        del temp_vector_dataSource, temp_vector_layer, temp_feature
