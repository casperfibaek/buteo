...
# TODO: Add tests for the stats dict
...
def test_scaler_standardise_mad():
    ...
    result, _stat_dict = scaler_standardise_mad(arr)
    expected_median = 0.0
    assert np.isclose(np.median(result), expected_median, atol=1e-6)

def test_scaler_iqr():
    """ Test the IQR scaler. """
    result, _stat_dict = scaler_iqr(arr)
    expected_median = 0.0
    assert np.isclose(np.median(result), expected_median, atol=1e-6)

def test_scaler_to_range():
    """ Test the to-range scaler. """
    result, _stat_dict = scaler_to_range(arr, min_val=-1.0, max_val=1.0)
    expected = np.array([-1.0, -0.5, 0.0, 0.5, 1.0], dtype=float)
    assert np.allclose(result, expected)

def test_scaler_truncate():
    """ Test the truncate scaler. """
    result, _stat_dict = scaler_truncate(arr, trunc_min=2.0, trunc_max=4.0, target_min=0.0, target_max=1.0)
...
def test_scaler_truncate_another_input():
    ...
    result, _stat_dict = scaler_truncate(arr * 2, trunc_min=4.0, trunc_max=8.0, target_min=0.0, target_max=1.0)
    expected = np.array([0.0, 0.0, 0.5, 1.0, 1.0], dtype=float)
    assert np.allclose(result, expected)
