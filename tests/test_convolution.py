...
def test_weight_distance():
    ...
    weights = _weight_distance(arr)

    assert weights == 1.0

    weights = _weight_distance(arr, method="linear", decay=0.5)
    assert np.isclose(weights, 0.5)

    weights = _weight_distance(arr, method="sqrt", decay=0.5)
    assert np.isclose(weights, np.sqrt(2) / 2)

    weights = _weight_distance(arr, method="power", decay=0.5)
    assert np.isclose(weights, 0.25)

    weights = _weight_distance(arr, method="log")
    assert np.isclose(weights, 1.0986122886681098)

    weights = _weight_distance(arr, method="gaussian", sigma=1)
    assert np.isclose(weights, 0.6065306597126334)

    with pytest.raises(ValueError):
        _weight_distance(arr, method="unknown")
...
def test_convolve_array_min():
    ...
    result = convolve_array(arr, offsets, weights, method=_METHOD_ENUMS["min"]) # min
...
def test_convolve_array_max():
    ...
    result = convolve_array(arr, offsets, weights, method=_METHOD_ENUMS["max"]) # max
...
def test_convolve_array_same_size_result():
    ...
    for method in _METHOD_ENUMS.values():
        _kernel, weights, offsets = get_kernel(3)
