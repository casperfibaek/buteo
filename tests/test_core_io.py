...
def test_array_to_raster_2D():
    ...
    result_path = core_io.array_to_raster(array, reference=reference_path)
...
def test_array_to_raster_3D_channel_last():
    ...
    result_path = core_io.array_to_raster(array, reference=reference_path, channel_last=True)
...
def test_array_to_raster_3D_channel_first():
    ...
    result_path = core_io.array_to_raster(array, reference=reference_path, channel_last=False)
...
def test_array_to_raster_nodata_arr():
    ...
    result_path = core_io.array_to_raster(array, reference=reference_path, set_nodata="arr")
    result_raster = gdal.Open(result_path)
    assert result_raster is not None
...
def test_array_to_raster_nodata_ref():
    ...
    result_path = core_io.array_to_raster(array, reference=reference_path, set_nodata="ref")
    result_raster = gdal.Open(result_path)
    assert result_raster is not None
...
def test_array_to_raster_bounding_box():
    ...
    result_path = core_io.array_to_raster(array, reference=reference_path, bbox=bbox)
    result_raster = gdal.Open(result_path)
    assert result_raster is not None
...
def test_raster_to_array_and_array_to_raster_identity():
    ...
    original_array = core_io.raster_to_array(reference_path)
    result_path = core_io.array_to_raster(original_array, reference=reference_path)
    result_array = core_io.raster_to_array(result_path)
...
def test_raster_to_array_and_array_to_raster_modified():
    ...
    result_path = core_io.array_to_raster(modified_array, reference=reference_path)
    result_array = core_io.raster_to_array(result_path)
...
def test_raster_to_array_and_array_to_raster_custom_nodata():
    ...
    result_path = core_io.array_to_raster(modified_array, reference=reference_path, set_nodata=-9999)
    result_array = core_io.raster_to_array(result_path)
...
def test_raster_to_array_and_array_to_raster_bbox():
    ...
    result_path = core_io.array_to_raster(sub_array, reference=reference_path, bbox=bbox)
    result_array = core_io.raster_to_array(result_path)
...
def test_raster_to_array_and_array_to_raster_pixel_offsets_identity():
    ...
    original_array = core_io.raster_to_array(reference_path, pixel_offsets=[2, 2, 6, 6])
    result_path = core_io.array_to_raster(original_array, reference=reference_path, pixel_offsets=[2, 2, 6, 6])
    result_array = core_io.raster_to_array(result_path)
...
def test_raster_to_array_and_array_to_raster_pixel_offsets_modified():
    ...
    original_array = core_io.raster_to_array(reference_path, pixel_offsets=[2, 2, 6, 6])
    modified_array = original_array * 2
    result_path = core_io.array_to_raster(modified_array, reference=reference_path, pixel_offsets=[2, 2, 6, 6])
    result_array = core_io.raster_to_array(result_path)
...
def test_raster_to_array_and_array_to_raster_pixel_offsets_custom_nodata():
    ...
    original_array = core_io.raster_to_array(reference_path, pixel_offsets=[2, 2, 6, 6])
    modified_array = np.where(original_array < 0.5, -9999, original_array)
    result_path = core_io.array_to_raster(modified_array, reference=reference_path, set_nodata=-9999, pixel_offsets=[2, 2, 6, 6])
    result_array = core_io.raster_to_array(result_path)
...
def test_raster_to_array_and_array_to_raster_pixel_offsets_overlapping():
    ...
    original_array = core_io.raster_to_array(reference_path, pixel_offsets=[4, 4, 6, 6])
    result_path = core_io.array_to_raster(original_array, reference=reference_path, pixel_offsets=[4, 4, 6, 6])
    result_array = core_io.raster_to_array(result_path)
...
def test_save_raster_to_disk():
    ...
    core_io.save_dataset_to_disk(in_path, out_path)
...
def test_save_dataset_to_disk_with_prefix():
    ...
    output_path = core_io.save_dataset_to_disk(in_path, out_path, prefix=prefix)
...
def test_save_dataset_to_disk_with_suffix():
    ...
    output_path = core_io.save_dataset_to_disk(in_path, out_path, suffix=suffix)
    assert os.path.exists(output_path)
...
def test_save_dataset_to_disk_no_overwrite():
    ...
    core_io.save_dataset_to_disk(in_path, out_path)
    core_io.save_dataset_to_disk(in_path, out_path, overwrite=False)
    assert os.path.exists(out_path)
    os.remove(out_path)
...
def test_single_chunk(sample_raster):
    chunk_gen = core_io.raster_to_array_chunks(sample_raster, chunks=1)
...
def test_multiple_chunks(sample_raster):
    chunk_gen = core_io.raster_to_array_chunks(sample_raster, chunks=4)
...
def test_overlap(sample_raster):
    chunk_gen = core_io.raster_to_array_chunks(sample_raster, chunks=4, overlap=10)
...
def test_filled_and_fill_value(sample_raster):
    chunk_gen = core_io.raster_to_array_chunks(
        sample_raster,
        chunks=1,
        filled=True,
        fill_value=0,
...
def test_numpy_to_raster(sample_raster):
    ...
    raster_result = core_io.array_to_raster(array, reference=sample_raster)

    array_all = core_io.raster_to_array(raster_result, bands=-1)
    assert np.array_equal(array_all, array)

    array_first = core_io.raster_to_array(raster_result, bands=[1])
    assert np.array_equal(array_first, array[:, :, 0:1])

    array_last = core_io.raster_to_array(raster_result, bands=[3])
    assert np.array_equal(array_last, array[:, :, 2:3])

    array_flip = core_io.raster_to_array(raster_result, bands=[3, 2, 1])
...
def test_create_raster_from_array_default():
    ...
    output_name = core_io.raster_create_from_array(arr)
...
def test_create_raster_from_array_special():
    ...
    output_name = core_io.raster_create_from_array(
        arr,
        pixel_size=0.5,
        x_min=10,
