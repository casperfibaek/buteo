...
def buildMosaic(scenes, band, md_dest, output_dir = os.getcwd(), output_name = 'mosaic', step = 2000, cloud_buffer = 0, processes = 1, percentile = 25., colour_balance = False, masked_vals = [0,1,2,3,7,8,9,10,11], output_mask = True, temp_dir = '/tmp', verbose = False, resampling = 0):
    ...
    for tile in np.unique([s.tile for s in scenes_sorted]):
        ...
        for n, block in enumerate(blocks):
            ...
            print('shape 1: ', composite_parts[n][0].shape)
            print('shape 2: ', composite[col:col + col_step, row:row + row_step].shape)

            if len(composite_parts[n][0].shape) == 3:
                print('shapes do not match!')
                if len(composite[col:col + col_step, row:row + row_step].shape) == 3:
                    composite[col:col + col_step, row:row + row_step][1:3] = composite_parts[n][0][1:3]
                    slc[col:col+col_step,row:row+row_step][1:3] = composite_parts[n][1][1:3]
                else:
                    composite[col:col + col_step, row:row + row_step] = composite_parts[n][0][1:3]
                    slc[col:col+col_step,row:row+row_step] = composite_parts[n][1][1:3]
            else:
