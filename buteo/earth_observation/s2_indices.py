...
def calc_indices(
    ...
    if index == "chlre":  # Red Edge Chlorophyll Index
        ...
        return ne.evaluate(
            "(B08 - b) / (B08 + b)", {"b": ne.evaluate("2 * B04 - B02")}, {"B08": B08}
        )
        ...
        return ne.evaluate(
            "(2 * B08 + 1 - sqrt((2 * B08 + 1) ** 2) - 8 * (B08 - B04)) / 2"
        )
        ...
        return ne.evaluate(
            "(B12 - d) / (B12 + d)",
            {"d": ne.evaluate("B08 / B02")},
            {"B12": B12, "B08": B08, "B02": B02},
        )
        ...
        return ne.evaluate(
            "(bix - B11) / (bix + B11)", {"bix": ne.evaluate("(B03 + B04 + B12) / 3")}
        )
    elif index == "ibi":  # Built-up features extraction
        return ne.evaluate(
            "(ndbi - ((savi + ndwi2) / 2)) / (ndbi + ((savi + ndwi2) / 2))",
            {
