...
def reproject(
    ...
    assert (
        reference_raster is None or target_projection is None
    ), "reference_raster and target_epsg cannot be applied at the same time."
        ...
        if isinstance(
            ...
            try:
                target_projection = CRS.from_wkt(
                    gdal.Open(reference_raster, gdal.GA_ReadOnly).GetProjection()
                )
            except:
    ...
    destination_dataframe = driver.Create(
        out_raster, cols, rows, 1, datatype, creation_options
    )
